...
Then the sources can be transformed into the new desired format. In this case we’re just adding the search query in the response to ease filtering on the client side. Then the streams are merged in a single Json stream using the `flatMapMerge` method (and thanks to the TweetInfo implicit Json formatter). In this case we're just adding the search query in the response to ease filtering on the client side. Finally, `EventSource.flow` method helps us to format the messages into the Server Sent Event format. And the stream can flow!  
