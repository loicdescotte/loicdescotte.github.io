...
The architecture makes a deliberate bet: **Loom removes the need for async types almost everywhere**. The `Future` boundary is pushed to the very edge — where Pekko HTTP hands off to the application — and the rest of the code is plain, readable, synchronous Scala. As Caprese matures, `Either` can gradually give way to `throws`, further simplifying return types while keeping the same compile-time safety guarantees.
